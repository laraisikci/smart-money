...
const RESPONSE_CACHE_TTL_MS = 2 * 60 * 60_000; // 2h, per spec — request coalescing below (see
// inFlight) is what actually protects against SEC rate limiting during the cache-miss window,
// more so than the TTL length itself, since the shared secClient throttle already serializes
// dispatch regardless of caller concurrency — the real risk was multiple *concurrent* cold-cache
// requests each independently kicking off a full scan.
...
// If a request arrives while a computation is already running (e.g. right after the cache
// expires, or during startup pre-warming), it awaits this same in-flight promise instead of
// kicking off a second full scan of the ticker universe. Without this, N concurrent cold-cache
// requests multiply the SEC request volume by N through the exact same shared throttle — this is
// what actually caused SEC EDGAR to start timing out on us during testing, not insufficient
// batching.
let inFlight: Promise<ResponseCacheEntry['value']> | null = null;

// Shared by the route handler and the startup pre-warm task (see lib/prewarm.ts) so both go
// through the exact same cache-check → coalesce → compute → store path.
export async function getCachedInsiders(): Promise<ResponseCacheEntry['value']> {
...
export function insidersRouter(): Router {
  ...
  router.get('/', async (_req, res) => {
    try {
      const value = await getCachedInsiders();
      res.json(value);
    } catch (err) {
